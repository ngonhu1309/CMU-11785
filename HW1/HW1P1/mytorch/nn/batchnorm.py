...
class BatchNorm1d:
    ...
    def backward(self, dLdBZ):
        ...
        dNZdM = -1 / np.sqrt(self.V + self.eps)  # TODO: Compute derivative of normalized input with respect to mean.
        dLdM = np.sum(dLdNZ * dNZdM, axis=0, keepdims=True) + dLdV * (-2 * np.mean(self.Z - self.M, axis=0, keepdims=True))  # TODO: Compute gradient of loss with respect to mean.

        dLdZ = dLdNZ / np.sqrt(self.V + self.eps) + dLdV * 2 * (self.Z - self.M) / self.N + dLdM / self.N  # TODO: Compute gradient of loss with respect to the input.
